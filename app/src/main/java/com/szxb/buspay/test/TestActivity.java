...
public class TestActivity extends BaseActivity implements OnReceiverMessageListener {
    ...
    protected void initData() {
        ...
        main_sign.setVisibility(View.GONE);
        boolean is = (boolean) CommonSharedPreferences.get("first_start_", true);
        if (is) {
            List<String> list = FileParse.readFileToListString(BuildConfig.FILE_NAME);
            List<PosSnEntity> posSnEntityList = new ArrayList<>();
            for (String s : list) {
                PosSnEntity pos = new PosSnEntity();
                pos.setPosSN(s);
                pos.setCnt(DateUtil.getCurrentDate("yyyy-MM-dd"));
                posSnEntityList.add(pos);
            }
            DBCore.getDaoSession().getPosSnEntityDao().insertOrReplaceInTx(posSnEntityList);

            CommonSharedPreferences.put("first_start_", false);
            BusApp.getTestPos().load();
        }
    }
    ...
    private void initDatas() {
        ...
        setPrices();
        station_name.setText(BusApp.getPosManager().getChinese_name() + "【" + BuildConfig.LOG_TAG + "】");
    ...
    protected void message(QRScanMessage message) {
        switch (message.getResult()) {
            ...
            case QRCode.KEY_CODE:
                ...
                break;
            case QRCode.SN:
                textView.setText("当前序号=" + BusApp.getTestPos().getSNID() +
                        "\n当前SN=" + message.getPosRecord().getPos_no());
                break;
            case QRCode.UN:
                textView.setText("二维码当前序号=" + BusApp.getTestPos().getSNID() +
                        "\n当前SN=" + message.getPosRecord().getPos_no() +
                        "\n银联当前序号=" + BusApp.getTestPos().getUnID() +
                        "\n当前SN=" + TestUtil.TA_Union_Pay_Pos[BusApp.getTestPos().getUnID() % TestUtil.TA_Union_Pay_Pos.length] +
                        "\n当前POS=" + TaiAnDate.posSn[BusApp.getTestPos().getUnID() % TaiAnDate.posSn.length]
